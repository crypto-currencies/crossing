...
// ─── Seed identity ──────────────────────────────────────────────────────────
// Two synthetic accounts, never real users. The demo account exists so you can
// log in and exercise save/vote/submit without hand-creating a throwaway user.
//
// Its password is GENERATED PER RUN and never committed (SEC-003).
...
// newest/trending sorts have something to demonstrate.
//
// ⚠️ FIXTURE ENGAGEMENT DATA (SEC-003)
// `votes`, `saves`, and `views` below are INVENTED. They exist solely to give
// the ranking formula non-trivial input in local development. They are not
// measurements of anything and must never be presented to a user as real
// engagement — which is why the production guard above exists, and why every
// row is attributed to an `@seed.crossing.dev` account so seeded rows remain
// identifiable in any database they reach.

/** Marks a numeric field as invented demo input rather than a measurement. */
type FixtureCount = number;
...
interface SeedListing {
  ...
  websiteUrl: string;
  daysAgo: number;
  /** Invented — see FIXTURE ENGAGEMENT DATA above. */
  votes: FixtureCount;
  saves: FixtureCount;
  views: FixtureCount;
}
...
async function main() {
  // Refuses outright in production unless SEED_ALLOW_PROD=true (SEC-003).
  assertSeedAllowed();

  console.log("Seeding Crossing.dev discovery domain (idempotent)...\n");

  const demo = resolveDemoPassword();

  // ── Seed identity ──
  // Both accounts are pinned to `role: USER` EXPLICITLY. Relying on the schema
  // default would mean a future change to that default silently promotes every
  // demo account to something privileged.
  ...
  const demoUser = await db.user.upsert({
    ...
    // Re-running rotates the password so a stale value never lingers.
    update: { role: "USER", passwordHash: hashPassword(demo.password) },
    create: {
  ...
  console.log(`  users: ${SEED_ATTRIBUTION_USER.email}, ${SEED_DEMO_USER.email}`);
  if (demo.generated) {
    // Written to a gitignored file, NOT stdout — a printed password would end up
    // in CI logs and terminal scrollback.
    await writeFile(
      DEMO_CREDENTIALS_FILE,
      `# Generated by prisma/seed.ts — gitignored, safe to delete.\n` +
        `email=${SEED_DEMO_USER.email}\npassword=${demo.password}\n`,
      { mode: 0o600 },
    );
    console.log(`  demo password: written to ${DEMO_CREDENTIALS_FILE} (not printed)`);
  } else {
    console.log("  demo password: taken from SEED_DEMO_PASSWORD");
  }
